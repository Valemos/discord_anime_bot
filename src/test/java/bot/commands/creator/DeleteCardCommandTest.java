...
class DeleteCardCommandTest {
    ...
    void testCardNotDeletedForIncorrectCommand() {
        List<CardGlobal> cards = new ArrayList<>(sender.getGame().getCardsGlobalManager().getAllCards());

        sender.send("#delcard");

        assertEquals(cards, sender.getGame().getCardsGlobalManager().getAllCards());
    }
    ...
    void testDeletedWhenNameUnique() {
        List<CardGlobal> cards = sender.getGame().getCardsGlobalManager().getAllCards();
        assertTrue(cards.contains(sender.cardGlobal1));

        sender.send("#delcard riko");

        cards = sender.getGame().getCardsGlobalManager().getAllCards();
